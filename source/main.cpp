...
int main()
{
	...
	for (string input_ = ""; input_ != "EXIT"; )
	{
			...
			{
				{
					...
					else
					{
						...
						do
						{
							std::cout << "Select a movie by typing their number (\"-1\" to return to main menu):";
							selected = size;
							std::cin >> selected;
							if (selected >= size)
							{
							...
							{
								std::cout << AllMovies_[selected]->getTitle() << "a" << AllMovies_[selected]->getGenre() << "movie\n";
								// DO INTERACTIONS
							}
						} while (selected >= 0);
		...
		else // did not recognize the command
		{
